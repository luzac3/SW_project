...
for($l=0; $l<count($list); $l++){
	$sql_name="SELECT name FROM ".$table_name." LIMIT 1 OFFSET ".($list[$l]-1);
	$result=$mysqli -> query($sql_name);
	foreach ($result as $row){
		$sword_array = explode("_",$list[$l]);
		if(count($sword_array)>1){
			$name_array[$l]=$row["name"]."(剣)";
			$list[$l]=$sword_array[0];
			$sword_piece[$l]="sword";
			//一応listの内容から不要なSwordのもじを削除
		}else{
			$name_array[$l]=$row["name"];
			$sword_piece[$l]="none";
		}
	}
}
...
function name_set($name_array,$name_num,$l){
	$i=0;
	$j=0;
	$k=0;
	$len=count($name_array);
	for($j=0; $j<$len; $j++){
		if($name_array[$j]==$name_array[$l] && $j!=$l){
			//何かが重複
			//同値は無視
			for($k=0; $k<$l; $k++){
				if($name_array[$k]==$name_array[$l] && $k!=$l){
				//同値なら無視するため、同値だとのまま
					$i++;
					//あれば増える
				}
			}
			return $name_array[$l].'_'.$name_num[$i];
		}else{
			continue;
		}
	}
		...
		foreach ($result as $row){
			...
			for($k=0; $k<count($status_booty); $k++){
				...
				if($status_booty[$k]=="name"){
					//ようするにネームはrowからではなくこちらから出力
					$name=name_set($name_array,$name_num,$l);
					//よってリストの送信は不要
					echo '<td class="_'.$status_booty[$k].'"><a class ="id_'.$row["ID"].'" name="'.$sword_piece[$l].'" href="#"; return false;>'.$name.'</a></td>';
