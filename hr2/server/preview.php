...
EOT;
$ex=explode($array["move"]);
if(count($ex)>1){
	echo "<td>".$ex[0]."／".$ex[1]."(".$ex[2].")</td></tr>";
}else{
	echo "<td>".$array["move"]."</td></tr>";
}
...
EOT;
$ex1=explode("_",$array["skill_region"]);
$ex2=explode("_",$array["skill_region"]);

echo "<td>";
	for($i=0; $i<count($ex1); $i++){
		echo "<p class='bottom_line'>";
		echo $ex1[$i];
		echo "</p>";
...
$ex1=explode($array["booty_dice"]);
$ex2=explode($array["booty"]);
$ex3=explode($array["booty_gamel"]);
$ex4=explode($array["booty_dice"]);
$len=count($ex);
